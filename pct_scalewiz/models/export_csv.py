...
def export_csv(project: Project) -> None:
    ...
    blanks = [test for test in project.tests if test.includeOnRep.get() and test.isBlank.get()]
    trials = [test for test in project.tests if test.includeOnRep.get() and not test.isBlank.get()]
    tests = blanks + trials
    ...
    output_dict["duration"] = [round(len(test.readings) * project.interval.get() / 60, 2) for test in tests]
    ...
    logger.info(f"Finished export of {project.name.get()} as {project.output_format.get()} in {round(time.time() - start_time, 3)} s")
