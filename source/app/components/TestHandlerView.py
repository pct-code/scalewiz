...
class TestHandlerView(ttk.Frame):
    ...
    def build(self):
        ...
        self.dev1Ent = ttk.Combobox(frame, width=15, textvariable=self.handler.dev1, values=self.devList)
        self.dev2Ent = ttk.Combobox(frame, width=15, textvariable=self.handler.dev2, values=self.devList)
        ...
        ttk.Label(self.trialLblFrm, text="Chemical:").grid(row=0, column=0, sticky=tk.E, pady=1)
        ttk.Label(self.trialLblFrm, text="Rate (ppm):").grid(row=1, column=0, sticky=tk.E, pady=1)
        ttk.Label(self.trialLblFrm, text="Clarity:").grid(row=2, column=0, sticky=tk.E, pady=1)
        ...
        rateEnt = ttk.Spinbox(self.trialEntFrm, textvariable=self.handler.test.rate, from_=0, to=999999)
        rateEnt.grid(row=1, column=0, sticky='ew', pady=1)
        clarity_options = ["Clear", "Slightly hazy", "Hazy"]
        clarityEnt = ttk.Combobox(self.trialEntFrm, values=clarity_options, textvariable=self.handler.test.clarity)
        ...
        self.startBtn = ttk.Button(frame, text="Start", command=lambda: self.handler.startTest())
        stopBtn = ttk.Button(frame, text="Stop", command=lambda: self.handler.requestStop())
        plotBtn = ttk.Button(frame, text="Toggle Details", command=lambda: self.update_PlotVisible())
